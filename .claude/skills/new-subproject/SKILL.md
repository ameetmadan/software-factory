...
"<piece>"`. If it doesn't exist yet, use the `roadmap` skill to create
   it first.
2. Design the piece: classify scope, ask clarifying questions, propose
   ...
   otherwise run the same process manually. Treat it as architectural —
   new subsystem, no existing flow in this repo to extend. Reuse the
   six-piece roadmap context from `AGENTS.md` and the epic issue's
   scope section; do not re-decompose pieces that already have their
   own epic.
